...
class CommonController extends Controller
{
  ...
  public function autoComplete(Request $request)
  {
    ...
    $filterQuoteArrayid = array();
    foreach ($client as $key => $value) {
      $filterQuoteArray[] =  $value->client_name;
      $filterQuoteArrayid[] =  $value->id;
      $filterQuoteArrayamount[] =  $value->amount;
     
    }
    //echo "<pre>IN_NO";($filterQuoteArray);exit;
    return response()->json(['error' => false, 'message' => "success",  "autocomplete" => $filterQuoteArray, 'id'=> $filterQuoteArrayid, "amount" => $filterQuoteArrayamount,], 200);
  }
  ...
  public function autoProduct(Request $request)
  {
    ...
    $product = product::where('product_name', 'like', '%' . $data . '%')->where('status','=',1)
    ...
    foreach ($product as $key => $value) {
      ...
      $filtergetQuantity[] = $value->total_quantity - $value->total_quantity_sold;
    
    }


    
    return response()->json(['error' => false, 'message' => "success", "product_details" => $filterArray, "product" => $filterProductArray, "description" => $filterDescription, 'price' =>$filtergetPrice, 'filtergetQuantity'=>$filtergetQuantity], 200);
  }
 ...
 /**
  ...
  public function saveExpense(Request $request){
     ...
     // echo"<pre>system";print_r($formData);exit();


            $Expense = Expense::create([
          "title" => $formData["title"],
           "amount" => $formData["amount"],
              "expense_no" => $formData["id"],
              "date" => date('Y-m-d'),
              "note" => $formData["note"],
              ]);
        ...
        return response::json(['status' => 1,'success' => false, 'message' => "Expense stored successfully"], 200);
...
public function expenses(){
  ...
  $expense =  Expense::select('title', 'amount', 'date','expense_no', 'note', 'id')->where('deleted_at', NULL)->get();
...
'note' => $value->note
   ...
   // echo"<pre>system";print_r($expenseArray);exit();

      return response::json(['status' => 1, 'message' => "success", "expense" => $expenseArray], 200);
 ...
 public function showexpense($id){
                ...
                $expenseCount = Expense::where('id', $id)->count();
        if ($expenseCount > 0) {
             $Expense = Expense::select('title', 'amount', 'date','expense_no', 'note', 'id')->first();
            $ExpenseArray = array();
            ...
            $ExpenseArray = [
                ...
                'title' => $Expense->title,
                   'id' => $Expense->expense_no,
                    'note' => $Expense->note,
                   
            ];
            return response()->json(['status' => 1, 'message' => "success", "ExpenseArray" => $ExpenseArray], 200);
        } else {
 ...
 public function updateExpense(Request $request, $id){
          ...
          $TaxRate = Expense::where('id', $id)->update($formdata);
          
return response::json(['status' => 1, 'message' =>"Expense Updated successfully"], 200);
