...
$current_year = $current_date->year;
  ...
  $currency = Currencies::select('symbol')->first();



   $loans = Loan::select('amount', 'paid_amount')
   ->where('date', 'LIKE', $current_year .'%')
   ->where('deleted_at', NULL)->get();

     $loan_disbursed_array = array();
     $loan_repayed_array = array();

  foreach ($loans as $keys => $values) {

    array_push($loan_disbursed_array, $values->amount);
    array_push($loan_repayed_array, $values->paid_amount);

   
    
}

$loan_disbursed =number_format(array_sum($loan_disbursed_array),2 );
$loan_repayment =number_format(array_sum($loan_repayed_array),2 );




  $item_purchase = invoice::select('total_paid_amount')
->where('date', 'LIKE', $current_year .'%')
  ->where('deleted_at', NULL)->get();


  $sum_item_purchase = array();

  foreach ($item_purchase as $keys => $values) {

    array_push($sum_item_purchase, $values->total_paid_amount);

   
    
}

$item_purchased =number_format(array_sum($sum_item_purchase),2 );


  $sum_item_disbursed= array();
 $item_disbursed = quotes_item_amount::select('total_amount')
->where('created_at', 'LIKE', $current_year .'%')
 ->get();

   foreach ($item_disbursed as $keys => $values) {

    array_push($sum_item_disbursed, $values->total_amount);

   
    
}

$item_disbursed =number_format(array_sum($sum_item_disbursed), 2);


  $contributors = Contributors_payment::select('payment_amount')
  ->where('payment_date', 'LIKE', $current_year .'%')
  ->get();

     //echo "<pre>";print_r($contributors);exit;

  $client = Client::Where('deleted_at', NULL)->get()->count();
...
foreach ($contributors as $keys => $values) {
 ...
 $dashboardnew = array();
   ...
   $expenses = Expense::select('expense_no', 'amount', 'title', 'date')
->where('date', 'LIKE', $current_year .'%')
   ->get();
...
$dashboardnew = [
 ...
 return response()->json(['status' => 1, 'message' => "success", 'dashboardnew' => $dashboardnew ],   200);
