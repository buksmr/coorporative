           ...
           <!-- <div>
            ...
            <div class="control is-hidden-mobile">
               <a class="button is-info is-small has-background-grey-darker" @click="filter">Search</a>
                      ...
                      <router-link :to="{name: 'loanform', query: {
        ...
        <b-table class="table is-bordered is-striped is-fullwidth search-table inner" 
         ...
         :default-sort-direction="defaultSortDirection" default-sort="">
            <template slot-scope="props">
               ...
               <b-table-column field="amount" class="is-size-7-mobile" label="Loan AMT" sortable>
                 {{props.row.currencies_placement==0 ? props.row.currencies_id : ''}}  {{props.row.amount}}
               </b-table-column>
                  <b-table-column field="amount_disbursed" class="is-size-7-mobile" label="Disbursed AMT" sortable>
                 {{props.row.amount_disbursed}}
               </b-table-column>
                  <b-table-column field="monthly_payment" class="is-size-7-mobile" label="Monthy AMT" sortable>
                {{props.row.monthly_payment}}
               </b-table-column>
                  ...
                  <b-dropdown hoverable>
                   ...
                   <router-link :to="{name: 'frontofficeform', query: { id : props.row.loan_id, edit: true }}" class="has-text-dark">
                     ...
                     <b-dropdown-item @click="savefiles(props.row.loan_id)">
                        ...
                        <a  @click="isImageModalActive1=true;  payment(props.row.loan_id, props.row.monthly_payment, props.row.balance_amount)"  class="has-text-dark">
...
this.setLoading(false);
       ...
       fetchquotes() {
         //  alert(1);
          this.setLoading(true);
            ...
            this.quotedata = response.data.loandetails;
          ...
          .finally(() => {
           ...
           this.quotes_id = id;
            let currentObj = this;
             ...
             axios({
                            url:"/loan_report/" + this.quotes_id,
                            method: "get",
         ...
         payment(id, payment, balance) {
             this.setLoading(true);
                this.payment_no();

            this.quotes_id = id;
             this.paymentBalance = Number(balance);
             this.item_payment.repayamount = Number(payment);
 ...
 this.item_payment.payment_no = 'PL'+Date.now().toString().substr(5, 7);
                ...
                context.$validator.validate().then(valid => {
                    if (valid) {
                     ...
                     this.isImageModalActive1 = false;
                        ...
                        let url = '/loanpayment/' + context.quotes_id;
                        let config = {
                            headers: {
